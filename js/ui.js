...
document.querySelectorAll('.r-btn').forEach(b=>b.onclick=()=>{document.querySelectorAll('.r-btn').forEach(x=>x.classList.remove('sel'));b.classList.add('sel')});
...
const role=document.querySelector('.r-btn.sel')?.dataset.r;
if(!name)return this.toast('Введи имя!',1);
if(!role)return this.toast('Выбери роль!',1);
...
grid.innerHTML=players.map(p=>`<div class="wp${p.ready?' wr':''}">${p.ready?'✅':'⏳'} ${p.name} (${p.role==='nursery'?'🐕':'🏪'})${p.isHost?' 👑':''}</div>`).join('');
